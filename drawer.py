...
pin_drawer_COM = 31  # COM 3.3-5v 采用共阳极
pin_drawer_DIR = 33  # DIR direction 方向信号 0V Vcom
pin_drawer_STP = 35  # STP 步进信号 100-20KHz
pin_drawer_EN = 37  # 脱机信号 Vcom使能电机 0V步进电机脱机状态
pins = (31, 33, 35, 37)

pin_NC = 13  # NC: pin13
pin_NO = 15  # NO: pin15

pin_NC_close_drawer = 13  # NC: pin7
pin_NO_close_drawer = 15  # NO: pin11
...
def test(first_flag: bool, close_flag: bool, pwm_tran):
    ...
    # 测试行程开关
    if GPIO.input(pin_NC_close_drawer) == 1 or GPIO.input(pin_NO_close_drawer) == 0:
        print("正在初始化...") if first_flag else None
        enable_or_disable(True)
        pwm.ChangeDutyCycle(50)

    elif GPIO.input(pin_NO_close_drawer) == 1 and GPIO.input(pin_NC_close_drawer) == 0:
...
if __name__ == '__main__':
    ...
    init()
    cw_or_ccw(False)
    pwm = GPIO.PWM(pin_drawer_STP, 800)  # 初始化PWM实例 频率为100-20kHz 1600 -> 1r/s
    pwm.start(0)  # 开始脉宽调制，参数范围为0->100
    o_first_flag = True
    cl_flag = False
    try:
        if GPIO.input(pin_NO_close_drawer) == 1 and GPIO.input(pin_NC_close_drawer) == 0:
            time.sleep(5)
            if GPIO.input(pin_NO_close_drawer) == 1 and GPIO.input(pin_NC_close_drawer) == 0:
                enable_or_disable(False)
                print("正处于初试状态，初始化完毕")
